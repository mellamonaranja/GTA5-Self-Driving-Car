...
def alexnet(width, height, lr):
    ...
    network = fully_connected(network, 4096, activation='tanh')
    network = dropout(network, keep_prob=0.5)
    network = fully_connected(network, 4, activation='softmax') #network, output...
...
def sentnet_color(width, height, frame_count, lr, output, model_name = 'sentnet_color.model'):
...
def inception_v3(width, height, frame_count, lr, output=3, model_name='sentnet_color.model'):
    ...
    # fc
    net = fully_connected(pool5_7_7, output, activation='softmax')

    #regression
    net = regression(net, optimizer='momentum', loss='categorical_crossentropy', learning_rate=lr,
                     name='targets')

    #model
    model = tflearn.DNN(net, checkpoint_path='model_sentnet_color',
                        max_checkpoints=1, tensorboard_verbose=2, tensorboard_dir='log')
